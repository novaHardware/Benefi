...
layout = html.Div([
    dcc.Markdown("""
        ### Predict
        Use the controls below to predict the expected months of employment based on job category, number of degreees obtained, Number of certifications obtained, past salary and months of employment in previous jobs.  
    ...
    html.Div([
        dcc.Markdown('###### Months of Employment'),
        dcc.Slider(
            id='months',
            min=0,
            max=1000,
...
def predict(category, degrees, certifications, salary, months):
    ...
    y_pred = y_pred_log[0]
    predict_months = y_pred
    results = f'We expect this person to stay employed for: "  {predict_months:.2f}'
